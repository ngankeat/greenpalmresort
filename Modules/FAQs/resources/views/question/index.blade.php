...
@section("breadcrumbs")
    <nav class="breadcrumb-style-one" aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">{{ __("menus.sidebar.faqs.question") }}</li>
        </ol>
    </nav>
@endsection
@section("actions")
    @isset($pageActions['action'])
        @foreach($pageActions['action'] as $action)
            @if($action['action_type'] == 'edit')
                <a href="{{ route("faqs.question.create") }}"><i data-lucide="list-plus"></i></a>
            @endif
        @endforeach
    @endisset
@endsection
                        ...
                        <form class="row row-cols-lg-auto g-3 align-items-center" id="filter">
                            <div class="col-12">
                                ...
                                <select class="form-select" id="inlineFormSelectCate" name="cboCategory">
                                    <option selected value="">{{ __("label.form.category.name") }}...</option>
