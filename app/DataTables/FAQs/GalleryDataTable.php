...
class GalleryDataTable extends DataTable
{
        ...
        return (new EloquentDataTable($query))
            ...
            ->editColumn('icon', function ($module) {
                ...
                if (!is_null($module->icon) AND !empty($module->icon)) {
                    return '<img style="width: 50px;" src="'.asset("faqs/icons/category/").'/'.$module->icon.'" />';
                } else {
                    return "";
                }
            })
            ...
            ->addColumn('action', function ($module) {
                return view('faqs::gallery.action', ['module' => $module]);
            })
            ->rawColumns(['icon', 'sound', 'soft_delete']);
    }

    /**
     * Get the query source of dataTable.
     */
    public function query(FAQsContact $model, Request $request): QueryBuilder
    {
        $model = $model->newQuery();
        /*if ($request->soft_delete) {
            if ($request->soft_delete == 'active_deleted') {
                $model->where('f_a_qs_categories.deleted_at', null);
            } elseif ($request->soft_delete == 'deleted') {
                $model->where('f_a_qs_categories.deleted_at', '!=', null);
            }
        }*/
        $model->withTrashed();
        $model->select([
            'f_a_qs_contacts.id',
            'f_a_qs_contacts.title',
            'f_a_qs_contacts.title_kh',
            'f_a_qs_contacts.description',
            'f_a_qs_contacts.email',
            'f_a_qs_contacts.phone']);
        return $model;
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('title')->title(__('label.table.th.title'))->width(50)->addClass('text-center'),
            Column::computed('title_kh')->title(__('label.table.th.title_kh'))->width(30)->addClass('text-center'),
            Column::make('description')->title(__('label.table.th.description')),
            Column::make('email')->title(__('label.table.th.email'))->width(60)->width(150)->addClass('text-center'),
            Column::make('phone')->title(__('label.table.th.phone'))->width(60)->width(150)->addClass('text-center'),
